...
void Potts::SW()
{
    ...
    for(int swp=0; swp<Natom; swp++)
    {
        ...
        int ncluster=0;//how many sites in the current cluster,1 represent the site i
...
void Potts::wolff()
{
    ...
    for(int swp=0; swp<Natom; swp++)
    {
        ...
        int q=floor(rand01()*Nq);
        int cluster[Natom];//record the current cluster
        int ncluster=1;//how many sites in the current cluster,1 represent the site i
...
void Potts::measure(int mcs)
{
    ...
    if(picture)
    {
        ...
        plot_title = sprintf('//dx//d Potts model with //d spins\nTemperature=//g,//s',Lx,Ly,Nq,T,text );
